            ...
            <div class="flex flex-col items-center justify-center gap-2 mb-4">
                <div class="w-full max-w-3xl mx-auto">
                    <flux:select wire:model.live="graph" :label="__('Change plot type')" variant="listbox">
                        ...
                        <flux:select.option :value="1">{{ __('Top Fold Change Features') }}</flux:select.option>
            ...
            @if ($this->differentialAbundanceTable)
                <div class="mt-4">
                    <div class="flex items-center justify-between mb-2">
                        <flux:heading size="md">{{ __('Differential Abundance Table') }}</flux:heading>
                        <div>
                            <flux:dropdown>
                                <flux:button icon:trailing="chevron-down">{{ __('Download') }}</flux:button>

                                <flux:menu>
                                    <flux:menu.item href="{{ $this->differentialAbundanceTableUrl }}">
                                        {{ __('Download all') }}
                                    </flux:menu.item>
                                    <flux:menu.item href="{{ $this->differentialAbundanceTablePVUrl }}">
                                        {{ __('Download p-Value filtered') }}
                                    </flux:menu.item>
                                    <flux:menu.item href="{{ $this->differentialAbundanceTableFDRUrl }}">
                            ...
                            <flux:table :paginate="$this->differentialAbundanceTable">
                                <flux:table.columns>
                                    ...
                                    <flux:table.column sortable
                                                       ...
                                                       :direction="$sortDirection"
                                                       wire:click="sort('logFoldChange')">{{ __('Log Fold Change') }}
                                    </flux:table.column>
                                    <flux:table.column sortable
                                                       :sorted="$sortBy === 'lfcSE'"
                                                       :direction="$sortDirection"
                                                       wire:click="sort('lfcSE')">{{ __('Std. Err.') }}
                                    </flux:table.column>
                                    <flux:table.column sortable
                                                       :sorted="$sortBy === 'pValue'"
                                                       :direction="$sortDirection"
                                                       wire:click="sort('pValue')">{{ __('p Value') }}
                                    </flux:table.column>
                                    <flux:table.column sortable
                                                       :sorted="$sortBy === 'padj'"
                                                       :direction="$sortDirection"
                                                       wire:click="sort('padj')">{{ __('FDR') }}
                                    </flux:table.column>
                                </flux:table.columns>
